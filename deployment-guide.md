...
4. Deploy your contract by running:
    ...
    Deployed SpxVrfy to 0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512
    ...
    ```
    ...
    >
    > ```bash
    > export SPX_VRFY_ADDRESS=$(jq -re '.transactions[] | select(.contractName == "SpxVrfy") | .contractAddress' ./broadcast/Deploy.s.sol/31337/run-latest.json)
    > ```
...
3. Deploy your contract by running:
    ...
    >
    > ```bash
    > export SPX_VRFY_ADDRESS=$(jq -re '.transactions[] | select(.contractName == "SpxVrfy") | .contractAddress' ./broadcast/Deploy.s.sol/11155111/run-latest.json)
    > ```
...
1. Query the state:
    ...
    ```bash
    cast call --rpc-url https://eth-sepolia.g.alchemy.com/v2/${ALCHEMY_API_KEY:?} ${SPX_VRFY_ADDRESS:?} 'get()(uint256)'
    ```
...
3. Deploy your contract by running:
    ...
    >
    > ```bash
    > export SPX_VRFY_ADDRESS=$(jq -re '.transactions[] | select(.contractName == "SpxVrfy") | .contractAddress' ./broadcast/Deploy.s.sol/1/run-latest.json)
    > ```
...
1. Query the state:
    ...
    ```bash
    cast call --rpc-url https://eth-mainnet.g.alchemy.com/v2/${ALCHEMY_API_KEY:?} ${SPX_VRFY_ADDRESS:?} 'get()(uint256)'
    ```
